...
export { SqliteTokenUsageStore, TOKEN_USAGE_APPLICATION_ID, TOKEN_USAGE_SCHEMA_VERSION, dayKey, openTokenUsageDatabase } from "./store.js";
/** Cordis plugin name. */
...
export function apply(ctx) {
    ...
    const openCaptures = new WeakMap();
    const openCaptureOf = (session) => openCaptures.get(session);
    const setOpenCapture = (session, open) => { openCaptures.set(session, open); };
    const clearOpenCapture = (session) => { openCaptures.delete(session); };
...
function recordFromStashedStep(session, fold, stash) {
    ...
    if (messageTime === null)
        throw new Error('token-usage: stashed record without a stamped message time');
